...
- **基準凍結の門＝2AIレビュー（着手前・必須）**：`criteria` / `verify` を確定（凍結）する瞬間に、
  **別系統の2つのAI（例：Claude と Codex）が同じリポを開いてレビューし、両方が「この分解と合格ラインで良い
  （atomic・十分・平易）」とサインするまで、実装（手順化）に進めない**。
  - レビュアーは**敵対的**（「なぜ atomic でない／不十分か」を探し、迷えば却下）。
  - **不一致は自動ブロックで終わらせず、両者の論拠を添えて人間（マスター）に上申**する
    （弱い判定者が対立する2つの論拠を読み比べて決める）。
  - サインは**当該 commit SHA に紐付く GitHub のPR承認**（偽造しにくい外部事実）。中身を変えたら旧承認は失効させる。
    門は branch protection で機械強制し、その設定内容はリポに文書化する（誰かが黙って門を外せないように）。
